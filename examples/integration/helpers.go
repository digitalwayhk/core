...
func StartProcess(options ProcessOptions) (*Suite, error) {
	...
	build := exec.Command("go", "build", "-race", "-o", binary, options.BuildPackage)
	build.Dir = repoRoot
	...
	if err := command.Start(); err != nil {
		_ = outputFile.Close()
		cleanup()
		return nil, fmt.Errorf("启动示例: %w", err)
	}

	servicePort := basePort + options.ServiceIndex
	return &Suite{
		RootDir:      rootDir,
		BasePort:     basePort,
		BaseURL:      "http://127.0.0.1:" + strconv.Itoa(servicePort),
		WebSocketURL: "ws://127.0.0.1:" + strconv.Itoa(servicePort) + "/ws",
		command:      command,
