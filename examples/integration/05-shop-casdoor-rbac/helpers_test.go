...
func startShopSuite() (*shopSuite, error) {
	...
	for _, name := range []string{"server.json", "casdoorrbacshop.json"} {
	...
	created.StopProcess()
	created.casdoor, err = newFakeCasdoor()
	if err != nil {
		created.Stop()
		return nil, err
	}
	if err := configureCasdoorSuite(created.RootDir, created.casdoor); err != nil {
		created.Stop()
		return nil, err
	}
...
// Stop 关闭商城进程、Fake Casdoor 和全部临时文件。
...
func (s *shopSuite) waitReady() error {
	...
	for time.Now().Before(deadline) {
		response, err := http.Get(s.BaseURL + "/api/casdoorrbacshop/getproducts")
		if err == nil {
...
// TokenFor 通过框架 Casdoor 配置和 callback 链路获取指定认证域 Token。
func (s *shopSuite) TokenFor(t testing.TB, subject string, tokenType int) string {
	t.Helper()
...
func (s *shopSuite) TokenPairFor(t testing.TB, subject string, tokenType int) integration.TokenResponse {
	t.Helper()
	authType := "auth"
	expectedClientID := authClientID
	if tokenType != 0 {
		authType = "manage"
		expectedClientID = manageClientID
	}
	configuration := s.RequestJSON(t, http.MethodGet, "/api/casdoor?type="+authType, "", nil)
	require.True(t, configuration.Success, configuration.ErrorMessage)
...
func (s *shopSuite) AddProductForSupplier(t testing.TB, token, code, name, price string, supplierID uint, enabled bool) ProductDTO {
	t.Helper()
	envelope := s.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/productmanage/add", token, map[string]interface{}{
...
func (s *shopSuite) AddSupplier(t testing.TB, token, code, name string, enabled bool) SupplierDTO {
	t.Helper()
	envelope := s.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/suppliermanage/add", token, map[string]interface{}{
...
func (s *shopSuite) SetBaseDataEnabled(t testing.TB, token, manageName, id string, enabled bool) {
	...
	envelope := s.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/"+manageName+"/"+command, token, map[string]interface{}{"id": id})
	require.True(t, envelope.Success, envelope.ErrorMessage)
}

func (s *shopSuite) AddPaymentType(t testing.TB, token, code, name string, enabled bool) PaymentTypeDTO {
	t.Helper()
	envelope := s.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymenttypemanage/add", token, map[string]interface{}{
...
func (s *shopSuite) AddOrder(t testing.TB, token string, productID uint, quantity int) OrderDTO {
	t.Helper()
	envelope := s.RequestJSON(t, http.MethodPost, "/api/casdoorrbacshop/addorder", token, map[string]interface{}{"productID": productID, "quantity": quantity})
	require.True(t, envelope.Success, envelope.ErrorMessage)
...
func (s *shopSuite) CreatePayment(t testing.TB, token, orderID, paymentTypeID string) OrderDTO {
	t.Helper()
	envelope := s.RequestJSON(t, http.MethodPost, "/api/casdoorrbacshop/createpayment", token, map[string]interface{}{
...
func (s *shopSuite) PaymentCommand(t testing.TB, token, command, paymentID string) OrderDTO {
	t.Helper()
	envelope := s.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymentrecordmanage/"+command, token, map[string]interface{}{"id": paymentID})
	require.True(t, envelope.Success, envelope.ErrorMessage)
	return decodeOrder(t, envelope.Data)
}
...
func (s *shopSuite) GetOrders(t testing.TB, token string) []OrderDTO {
	t.Helper()
	envelope := s.RequestJSON(t, http.MethodGet, "/api/casdoorrbacshop/getorders", token, nil)
	require.True(t, envelope.Success, envelope.ErrorMessage)
	var result []OrderDTO
	require.NoError(t, json.Unmarshal(envelope.Data, &result))
	return result
...
func (s *shopSuite) ConnectAndSubscribe(t testing.TB, token string) *websocket.Conn {
	...
	s.WriteWebSocket(t, connection, "sub", "/api/casdoorrbacshop/getorders", map[string]interface{}{})
...
func (s *shopSuite) ReadOrderEvent(t testing.TB, connection *websocket.Conn) OrderDTO {
	...
	require.Equal(t, "/api/casdoorrbacshop/getorders", message.Channel)
