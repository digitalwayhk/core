...
func TestManageAPIs(t *testing.T) {
	...
	t.Run("IdentityEventAuditReadOnly", testIdentityEventAuditReadOnly)
}
...
func testIdentityEventAuditReadOnly(t *testing.T) {
	...
	search := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/identityeventmanage/search", admin,
		map[string]int{"page": 1, "size": 20})
	require.True(t, search.Success, search.ErrorMessage)
	for _, command := range []string{"add", "edit", "remove"} {
		response := suite.RequestJSON(t, http.MethodPost,
			"/api/manage/casdoorrbacshop/identityeventmanage/"+command, admin, map[string]interface{}{})
...
func testRejectEnablingProductForDisabledSupplier(t *testing.T) {
	...
	enabled := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/productmanage/enablebasedata", admin, map[string]interface{}{"id": product.ID})
...
func testSupplierCRUDAndReadOnlyProducts(t *testing.T) {
	...
	childSearch := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/suppliermanage/search", token, map[string]interface{}{
	...
	edited := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/suppliermanage/edit", token, map[string]interface{}{
	...
	removed := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/suppliermanage/remove", token, map[string]interface{}{"id": supplier.ID})
	...
	removed = suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/suppliermanage/remove", token, map[string]interface{}{"id": removable.ID})
...
func testRejectRemovingUsedSupplier(t *testing.T) {
	...
	removed := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/suppliermanage/remove", admin, map[string]interface{}{"id": supplier.ID})
	assert.False(t, removed.Success)
	assert.Contains(t, removed.ErrorMessage, "供应商已有商品")
}
...
func testProductCRUD(t *testing.T) {
	...
	edited := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/productmanage/edit", token, map[string]interface{}{
		"id": product.ID, "code": product.Code, "name": product.Name + "-改", "price": "12.00", "supplierID": product.SupplierID, "enabled": true,
	})
	require.True(t, edited.Success, edited.ErrorMessage)
	removed := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/productmanage/remove", token, map[string]interface{}{"id": product.ID})
	require.True(t, removed.Success, removed.ErrorMessage)
}
...
func testRejectRemovingUsedProduct(t *testing.T) {
	...
	removed := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/productmanage/remove", admin, map[string]interface{}{"id": product.ID})
	assert.False(t, removed.Success)
...
func testPaymentTypeCRUDAndStateCommands(t *testing.T) {
	...
	disabled := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymenttypemanage/disablebasedata", admin, map[string]interface{}{"id": item.ID})
	require.True(t, disabled.Success, disabled.ErrorMessage)

	edited := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymenttypemanage/edit", admin, map[string]interface{}{
		"id": item.ID, "code": item.Code, "name": "测试支付已修改", "enabled": false, "description": "新说明",
	})
	require.True(t, edited.Success, edited.ErrorMessage)
	removed := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymenttypemanage/remove", admin, map[string]interface{}{"id": item.ID})
	require.True(t, removed.Success, removed.ErrorMessage)
}
...
func testRejectRemovingUsedPaymentType(t *testing.T) {
	...
	removed := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymenttypemanage/remove", admin, map[string]interface{}{"id": typeItem.ID})
	assert.False(t, removed.Success)
	assert.Contains(t, removed.ErrorMessage, "支付类型已被支付流水使用")

	changedCode := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymenttypemanage/edit", admin, map[string]interface{}{
...
func testOrderManageReadOnly(t *testing.T) {
	...
	for _, command := range []string{"add", "edit", "remove"} {
		response := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/ordermanage/"+command, admin, map[string]interface{}{})
		assert.Equal(t, http.StatusNotFound, response.HTTPStatus)
	}
}
...
func testPaymentRecordCommands(t *testing.T) {
	...
	search := suite.RequestJSON(t, http.MethodPost, "/api/manage/casdoorrbacshop/paymentrecordmanage/search", admin, map[string]int{"page": 1, "size": 100})
	require.True(t, search.Success, search.ErrorMessage)
	var table struct {
