...
func testGetProductsCombinesFiltersWithAND(t *testing.T) {
	...
	codeAndName := "/api/performanceshop/getproducts?code=" + url.QueryEscape(productA.Code) + "&name=" + url.QueryEscape("商品 A")
	...
	supplierAndName := "/api/performanceshop/getproducts?supplierID=" + url.QueryEscape(supplierA.ID) + "&name=" + url.QueryEscape("商品 B")
...
func testGetSuppliers(t *testing.T) {
	...
	response := suite.RequestJSON(t, http.MethodGet, "/api/performanceshop/getsuppliers?code="+url.QueryEscape(code), "", nil)
	require.True(t, response.Success, response.ErrorMessage)
...
func testGetProducts(t *testing.T) {
	...
	response := suite.RequestJSON(t, http.MethodGet, "/api/performanceshop/getproducts?name="+url.QueryEscape(name), "", nil)
	require.True(t, response.Success, response.ErrorMessage)
	var items []ProductDTO
	require.NoError(t, json.Unmarshal(response.Data, &items))
	require.Len(t, items, 1)
...
func testSupplierDisableControlsProductVisibility(t *testing.T) {
	...
	path := "/api/performanceshop/getproducts?supplierCode=" + url.QueryEscape(supplier.Code)
...
func testGetPaymentTypes(t *testing.T) {
	...
	response := suite.RequestJSON(t, http.MethodGet, "/api/performanceshop/getpaymenttypes?code="+url.QueryEscape(code), "", nil)
	require.True(t, response.Success, response.ErrorMessage)
