...
func TestPrivateAPIs(t *testing.T) {
	...
	t.Run("OrderCacheInvalidatesAfterMutation", testOrderCacheInvalidatesAfterMutation)
...
func testOrderCacheInvalidatesAfterMutation(t *testing.T) {
	admin := suite.TokenFor(t, "cache-order-admin", 1)
	user := suite.TokenFor(t, "cache-order-user", 0)
	product := suite.AddProduct(t, admin, fmt.Sprintf("缓存订单商品-%d", time.Now().UnixNano()), "13.00")
	require.Empty(t, suite.GetOrders(t, user), "首次查询应缓存空订单列表")

	order := suite.AddOrder(t, user, uintID(t, product.ID), 1)
	assert.Contains(t, orderIDs(suite.GetOrders(t, user)), order.ID, "下单成功后必须立即清理当前用户缓存")

	deleted := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/deleteorder", user, map[string]interface{}{"id": order.ID})
	require.True(t, deleted.Success, deleted.ErrorMessage)
	assert.NotContains(t, orderIDs(suite.GetOrders(t, user)), order.ID, "删除成功后必须立即清理当前用户缓存")
}
...
func testRejectCrossUserOrderMutations(t *testing.T) {
	...
	deleted := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/deleteorder", userB, map[string]interface{}{"id": order.ID})
	assert.False(t, deleted.Success)
	assert.Contains(t, deleted.ErrorMessage, "订单不存在或无权操作")

	paidByOther := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/createpayment", userB, map[string]interface{}{
	...
	cancelledByOther := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/cancelorder", userB, map[string]interface{}{"id": order.ID})
...
func testAddOrder(t *testing.T) {
	...
	unauthorized := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/addorder", "", map[string]interface{}{"productID": uintID(t, product.ID), "quantity": 1})
...
func testRejectDisabledSupplierOrder(t *testing.T) {
	...
	rejected := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/addorder", user, map[string]interface{}{"productID": uintID(t, product.ID), "quantity": 1})
...
func testCreatePayment(t *testing.T) {
	...
	order := suite.AddOrder(t, user, uintID(t, product.ID), 1)
	rejected := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/createpayment", user, map[string]interface{}{"orderID": order.ID, "paymentTypeID": disabled.ID})
	assert.False(t, rejected.Success)
	paying := suite.CreatePayment(t, user, order.ID, enabled.ID)
	assert.Equal(t, "支付中", paying.PaymentStatusName)
	second := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/createpayment", user, map[string]interface{}{"orderID": order.ID, "paymentTypeID": enabled.ID})
	assert.False(t, second.Success)
	deleteAttempt := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/deleteorder", user, map[string]interface{}{"id": order.ID})
...
func testWebSocketPaymentFlow(t *testing.T) {
	...
	cancel := suite.RequestJSON(t, http.MethodPost, "/api/performanceshop/cancelorder", userA, map[string]interface{}{"id": order.ID})
