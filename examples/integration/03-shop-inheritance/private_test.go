...
func testAddOrder(t *testing.T) {
	...
	unauthorized := suite.RequestJSON(t, http.MethodPost, "/api/inheritanceshop/addorder", "", map[string]interface{}{"productID": uintID(t, product.ID), "quantity": 1})
...
func testRejectDisabledSupplierOrder(t *testing.T) {
	...
	rejected := suite.RequestJSON(t, http.MethodPost, "/api/inheritanceshop/addorder", user, map[string]interface{}{"productID": uintID(t, product.ID), "quantity": 1})
...
func testDeleteOrder(t *testing.T) {
	...
	deleted := suite.RequestJSON(t, http.MethodPost, "/api/inheritanceshop/deleteorder", user, map[string]interface{}{"id": order.ID})
...
func testCreatePayment(t *testing.T) {
	...
	order := suite.AddOrder(t, user, uintID(t, product.ID), 1)
	rejected := suite.RequestJSON(t, http.MethodPost, "/api/inheritanceshop/createpayment", user, map[string]interface{}{"orderID": order.ID, "paymentTypeID": disabled.ID})
	assert.False(t, rejected.Success)
	paying := suite.CreatePayment(t, user, order.ID, enabled.ID)
	assert.Equal(t, "支付中", paying.PaymentStatusName)
	second := suite.RequestJSON(t, http.MethodPost, "/api/inheritanceshop/createpayment", user, map[string]interface{}{"orderID": order.ID, "paymentTypeID": enabled.ID})
	assert.False(t, second.Success)
	deleteAttempt := suite.RequestJSON(t, http.MethodPost, "/api/inheritanceshop/deleteorder", user, map[string]interface{}{"id": order.ID})
...
func testWebSocketPaymentFlow(t *testing.T) {
	...
	cancel := suite.RequestJSON(t, http.MethodPost, "/api/inheritanceshop/cancelorder", userA, map[string]interface{}{"id": order.ID})
