...
func TestPaymentFlowSupportsFailureRetryAndRefund(t *testing.T) {
	...
	assert.Equal(t, models.OrderStatusCancelling, refunding.Order.Status)
	...
	assert.Equal(t, models.OrderStatusCancelled, refunded.Order.Status)
	...
	assert.Equal(t, models.PaymentStatusFailed, records[0].Status)
	assert.Equal(t, models.PaymentStatusRefunded, records[1].Status)
