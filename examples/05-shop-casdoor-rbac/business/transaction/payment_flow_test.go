...
func TestPaymentFlowSupportsFailureRetryAndRefund(t *testing.T) {
	...
	_, err = basedatabusiness.NewPaymentTypeService().Disable(paymentType.ID)
	require.NoError(t, err)
	...
	require.ErrorContains(t, basedatabusiness.NewProductService().EnsureRemovable(product.ID), "商品已被订单使用")
	require.ErrorContains(t, basedatabusiness.NewPaymentTypeService().EnsureRemovable(paymentType.ID), "支付类型已被支付流水使用")
	require.ErrorContains(t, product.RemoveValid(), "只能禁用")
	require.ErrorContains(t, supplier.RemoveValid(), "只能禁用")
	require.ErrorContains(t, paymentType.RemoveValid(), "只能禁用")
	...
	_, err = basedatabusiness.NewPaymentTypeService().Enable(paymentType.ID)
	require.NoError(t, err)
