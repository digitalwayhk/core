...
func (own *PaymentService) CreatePayment(userID string, orderID, paymentTypeID, paymentID uint) (*PaymentChange, error) {
	...
	err := models.RunInTransaction(func(action persistencetypes.IDataAction) error {
		...
		if order.OrderStatus() != models.OrderStatusNormal {
...
func (own *PaymentService) finishPending(paymentID uint, success bool) (*PaymentChange, error) {
	...
	err := models.RunInTransaction(func(action persistencetypes.IDataAction) error {
		...
		if payment.PaymentStatus() == target {
			order, err = models.NewOrder().FindByIDWith(action, payment.OrderID)
			return err
		}
		if payment.PaymentStatus() != models.PaymentStatusPending {
...
func (own *PaymentService) ConfirmRefund(paymentID uint) (*PaymentChange, error) {
	...
	err := models.RunInTransaction(func(action persistencetypes.IDataAction) error {
		...
		if payment.PaymentStatus() == models.PaymentStatusRefunded && order != nil && order.OrderStatus() == models.OrderStatusCancelled {
			return nil
		}
		if order == nil || payment.PaymentStatus() != models.PaymentStatusRefunding || order.PaymentStatus != models.PaymentStatusRefunding {
			return models.NewBusinessError("只有退款中的流水可以确认退款")
		}
		now := time.Now().UTC()
		payment.Status = int(models.PaymentStatusRefunded)
