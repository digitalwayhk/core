...
func (own *PaymentTypeService) ValidateUpdate(item, old *models.PaymentType) error {
	...
	return own.EnsureUsedCodeStable(item, old)
}

// EnsureUsedCodeStable 阻止修改已产生支付流水的支付类型编码。
func (own *PaymentTypeService) EnsureUsedCodeStable(item, old *models.PaymentType) error {
	if item == nil || old == nil {
		return models.NewBusinessError("支付类型不存在")
	}
...
func (own *PaymentTypeService) EnsureRemovable(paymentTypeID uint) error {
	...
	item.SetID(paymentTypeID)
	return item.RemoveValid()
}
