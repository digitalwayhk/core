...
func NewIdentityEventRecord() *IdentityEventRecord {
	return &IdentityEventRecord{BusinessModel: NewBusinessModel(0)}
}
...
func (own *IdentityEventRecord) NewModel() {
	if own.BusinessModel == nil || own.ShopModel == nil || own.Model == nil {
		own.BusinessModel = NewBusinessModel(0)
	}
}
...
func (own *IdentityEventRecord) Normalize() error {
	if own == nil {
		return NewBusinessError("身份事件不能为空")
	}
	...
	switch {
	case own.EventID == "":
		return NewBusinessError("事件 ID 不能为空")
	case own.AuthType == "":
		return NewBusinessError("认证域不能为空")
	case own.UserID == "":
		return NewBusinessError("用户 ID 不能为空")
	case own.EventType == "":
		return NewBusinessError("事件类型不能为空")
	case own.OccurredAt.IsZero():
		return NewBusinessError("事件发生时间不能为空")
	}
