...
func NewOrder() *Order {
	return &Order{BusinessModel: common.NewBusinessModel(int(OrderStatusNormal)), PaymentStatus: PaymentStatusUnpaid}
}

// NewModel 供 ModelList 反射创建订单时初始化完整继承链。
func (own *Order) NewModel() {
	if own.BusinessModel == nil || own.ShopModel == nil || own.Model == nil {
		own.BusinessModel = common.NewBusinessModel(int(OrderStatusNormal))
	}
}
