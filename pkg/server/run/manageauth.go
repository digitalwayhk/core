...
func resolveManageAuthAuthority(
	...
	if len(eligible) > 1 && strings.TrimSpace(configured) == "" {
		return nil, errors.New("多个 Manage 服务必须通过 SetManageAuthAuthority 显式选择权威服务")
	}
...
func selectManageAuthContext(
	...
	return nil, fmt.Errorf("SetManageAuthAuthority 指定的服务不存在或未注册 Manage Router：%s", strings.TrimSpace(configured))
}
...
func validateManageAuthCompatibility(
	...
	if authority == nil || authority.Config == nil || authority.Service == nil {
