...
`/ws` 只识别三个事件，其余事件一律返回 `不支持的事件类型`：

| 事件 | 用途 |
|------|------|
| `sub` | 订阅 `channel` 指定的路由，或订阅 `logon`/`status`/`logout` 管理会话身份 |
| `unsub` | 取消订阅 |
| `get` | 查询当前会话的订阅信息 |

WebSocket 不提供「调用一次业务接口并返回结果」的事件。需要请求/响应语义时使用 HTTP 路由，走完整的认证与限流中间件链；WebSocket 只负责订阅与推送。`sub` 是唯一会执行路由代码的事件，它对 `Auth=true` 和 Private 路由在解析请求参数之前强制校验会话身份。
