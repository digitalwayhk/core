...
// ISubmitHook 为 Submit 状态转换提供类型安全的可选 Hook。
// 未实现该接口时，Submit 继续回退到通用 DoBefore 生命周期。
type ISubmitHook[T persistencetypes.IModel] interface {
	OnSubmit(item *T, req servertypes.IRequest) error
}

// IReleaseHook 为 Release 状态转换提供类型安全的可选 Hook。
// 未实现该接口时，Release 继续回退到通用 DoBefore 生命周期。
type IReleaseHook[T persistencetypes.IModel] interface {
	OnRelease(item *T, req servertypes.IRequest) error
}
