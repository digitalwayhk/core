...
面向 AI Agent 协作开发的 Go 业务服务框架。它不做通用技术框架，而是把商业服务的开发方式**标准化**：业务代码只表达「做什么、何时做」，「怎么做」交给框架，并且可以独立替换升级而不动已实现的业务逻辑。
...
- Go 版本以 [go.mod](./go.mod) 为准。核心 HTTP、配置、日志和生命周期复用 go-zero，SQL 模型契约用 GORM。
- 框架只保留 Digitalway 领域契约，不重复实现通用客户端、日志或并发原语。
- 外部依赖测试默认跳过，通过显式环境变量或 Docker Compose 启用。

## 60 秒跑通第一个服务

首次运行会在可执行文件目录自动生成 `etc/server.json` 和 `etc/shop.json`，**不需要手工准备配置，也不需要建库建表**：
...
首次启动的两个常见现象：内部 gRPC 默认占用 18080，被占用时会启动失败，改 `etc/server.json` 的 `Transport.GRPC.Port` 即可；未部署 Redis 时会打印若干连接失败日志然后降级为进程内事件（`mq_degraded`），不影响本示例。

另开一个终端，取测试令牌并走通一次下单。注意 `data` 是令牌对象而非字符串，要取 `data.access_token`：
...
# 2. 取普通用户令牌（type=0 用户 / 1 管理 / 2 服务管理）
...
路由默认是 `POST`，声明 `router.WithMethod(http.MethodGet)` 才是 `GET`。`TestToken` 无需登录，但只接受本地 IP 请求，生产必须在网关屏蔽 `/api/servermanage/*`。完整接口清单与 WebSocket 订阅见 [最简商城示例](./examples/01-simple-shop)。
...
成熟度分级（Stable / Conditional / Experimental / Unsupported）、必需配置和完整场景矩阵见 [框架场景使用指南](./docs/codex/FRAMEWORK_USAGE_GUIDE.md)。**配置字段存在不代表能力可用**，以校验、factory、启动链和行为测试为准。
...
**路径由包目录决定，`public`/`private` 不出现在 URL 里：**
...
**约定消除自由发挥的空间。** Agent 最容易出错的地方是「同一件事有多种写法」，上文速查表里的位置全部被固定死：包路径决定认证域、建表补列自动完成、一次 `NewManageService[T]` 出七个标准操作。

**产出可自检。** `GET /api/openapi` 输出业务契约，`GET /api/servermanage/testtoken` 签发三类认证域的测试令牌，Agent 写完即可自行发真实请求验证，不必等人工联调。

## AI 助手与 Skill（消费方）

`go get` **只安装 Go 模块**，不会把 Agent skill 装进业务仓库。消费方（及其 AI）需要先安装 skill，再按规范写代码。当 `go.mod` 依赖 `github.com/digitalwayhk/core`，或任务要求「按 Core 规范」开发时，Agent **应先检测再安装**，不要因找不到 skill 就凭记忆编码：
...
安装后 Agent **必须**阅读：消费方仓库内 `.codex/skills/use-digitalway-core/SKILL.md`（指针）、权威源 `.codex/skills/core-skill/SKILL.md` 及其「主题分片索引」中按任务需要的分片，以及 Core 源码里的 `docs/codex/*` 和最近 `examples/*`。Claude、Cursor、Copilot 的 skills 目录同构安装，各自指针都指向同一份权威源。

脚本加 `--write-agents` 可自动向消费方 `AGENTS.md` / `Claude.md` 追加标准段落。完整说明、软链/复制、用户级安装与故障排查见 [消费方 AI Skill 安装与识别](./docs/codex/CONSUMER_AI_SKILL_SETUP.md)。本仓库自身的 Agent 约定见 [AGENTS.md](./AGENTS.md)。
...
- 查询侧 L1 缓存按进程有效内存的 2% 自动解析字节预算（下限 16 MiB、上限 256 MiB），可选 Badger L2，同键冷加载用 SingleFlight 合并；数据变更后由事件主动失效，TTL 只兜底。

write-behind 是 at-least-once，远端 insert/update/delete 必须靠稳定主键或 upsert 保证幂等。同 key 多次更新会合并为最新状态，只适用于账户快照、资料和订单当前状态；资金流水、审计记录等不可合并事件必须走 transactional outbox 或带唯一事件 ID 的 JetStream。

## 多服务与水平扩展

- 只供其他服务调用的 Public API 声明 `router.WithInternalCallers("shop-user", ...)`。调用方身份只能来自同进程 ServiceContext 或已验证的 mTLS 证书 SAN，不接受 HTTP Header 或请求体自报。调用方直接构造目标服务已注册的 API 并用 `req.CallService`，不另建地址型 client。
- 内部同步调用默认走 gRPC，服务发现用 ServiceResolver（Redis / etcd / Consul），异步事件统一走 EventBridge。
- 数据库按业务域拆分，不按技术实例拆分。同一逻辑服务的多个副本共享一个远程权威库，每个副本持有独立的本地 pending store 用于可靠接收、崩溃恢复和批量同步；`AutoMachineID=true` 在扩容时自动分配唯一实例编号。
- 运行拓扑与请求聚合通过 `POST /api/servermanage/runtimetopology` 和 `runtimeservice` 查询，历史指标源是 go-zero / Prometheus。
...
**消费项目版本要求**：`go.mod` 使用的 Core 版本需包含 `pkg/persistence/entity/stats`，且 Core 嵌入的 `web/admin` 与分析、报表 JSON 契约匹配。当前分析与报表能力位于 `core-web-admin` 开发分支，尚未进入稳定 tag，不能仅执行 `go get ...@latest` 后假定可用；正式消费应等待包含该能力的发布版本，或在明确评审和锁定提交的前提下使用对应开发版本。不得通过复制 Core 实现或长期 `replace` 伪装成已发布能力。
...
公共 API 按 additive 方式演进。已废弃但仍可编译的表面统一登记在 [废弃 API 登记](./docs/codex/DEPRECATION_REGISTER.md)，逐条给出替代入口、最早删除版本、Owner 和迁移证据。表中「最早删除版本」是下限而非自动删除指令：删除前还需仓库内调用清零、已登记消费方完成迁移、CHANGELOG Removed 段完整。**升级 Core 版本前先对照该表检查自己用到的入口。**
...
CI 门禁由 `scripts/ci.sh` 按名称执行：`required/quick`、`required/contracts`、`required/ai-skill`（校验 AI skill 权威源结构）、`required/server-manage`、`required/race`。
