...
- Kafka 与 RabbitMQ 内建 MQ Provider：`MQ.Provider=kafka|rabbitmq` 可由 factory 直接构造，两者实现普通发布订阅和 `ReliableMQProvider`，发布等待 Broker 确认，可靠 Handler 成功后才 ACK。新增 Kafka TLS/SASL、RabbitMQ TLS/prefetch/queue prefix 配置、真实 Broker/EventBridge 外部门禁；两者不声明 `OrderedReliableMQProvider` 或 `KeyedReliableMQProvider`，显式 `KeyConcurrency>1` 必须 fail closed，业务仍需 EventID/Inbox 幂等。Kafka 可靠订阅在 Fetch/Commit 失败后重建 reader；新消费组默认 `StartOffset=latest`；GroupID 走资源名规范化；首次 Publish/Subscribe 声明 topic，Subscribe 返回时消费拓扑已就绪。
