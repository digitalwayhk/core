...
- Kafka 与 RabbitMQ 内建 Provider 实现 `MQProvider` 和 `ReliableMQProvider`：发布等待 Broker 确认，可靠 Handler 成功后才 ACK；失败可重投，语义是 at-least-once，不是 exactly-once。两者不实现 `OrderedReliableMQProvider` 或 `KeyedReliableMQProvider`，声明 ordered-reliable requirement 或显式 `KeyConcurrency>1` 时必须 fail closed，不得静默串行；业务仍以 EventID/Inbox 幂等。Kafka 新消费组默认 `StartOffset=latest`（接近 Redis `$`），只有显式 `earliest` 才从 topic 开头回放；Connect 只验证 Broker 可达，writer 在 Broker 允许时按动态 subject 自动建 topic，关闭 `auto.create.topics` 时由运维按 Prefix+规范化 subject 预建。Kafka GroupID 与 topic 一样走资源名规范化。Kafka 可靠订阅在 Fetch/Commit 失败后指数退避重建 reader，取消后不再重试。
