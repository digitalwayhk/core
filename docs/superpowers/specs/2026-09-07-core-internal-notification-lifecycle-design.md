...
状态：用户已选择方案 1；本文待确认，尚未实现。目标发布为 `v1.1.1`，以兼容性和真实集成验证通过为前提。
...
- 重新订阅后读取权威 generation 并清空本地缓存，确认本轮恢复期间没有再次失效后才能恢复加速。
...
沿用低基数日志/指标惯例，记录通道状态、失效原因分类、发布失败、补偿成功/失败、补偿耗时、队列压力及受保护会话数量。不得加入 UserID、消息 ID、token 或 payload 标签。未采集指标明确为未采集，不填假零。
