...
- `ApplyDefaults` 只为名为 `server` 的旧配置补齐 SQLite 默认值；业务服务配置若显式出现 `ManageStore`，`Validate` 必须拒绝，防止多份权威配置漂移。
- `Driver` 第一版只接受 `sqlite|mysql`；其他值在启动时 fail closed。
- 缺失 `ManageStore` 的旧 `server.json` 经 `ApplyDefaults()` 解析为 SQLite，不破坏旧部署。
...
内置 `core.system_admin` 和 `core.viewer` 的授权策略仍由代码动态计算。为了在角色管理页展示，启动初始化会幂等保证两条内置角色目录记录存在；不为内置角色创建权限明细行。
...
etc/server.json
  ...
  → ControlPlaneRuntime
       ├─ SystemManage 目录/菜单/按钮/角色/权限 CRUD
       ├─ 菜单同步与角色权限绑定事务
...
HTMLServer 为多服务选定 `ManageAuthAuthorityService` 时，权威服务的 `ManageRoleProvider` 与 Core `SystemManage` 共用上述 Authorizer。授权目标仍使用实际业务 RouterInfo 的 `service + path + command`。
...
4. 控制面五类模型使用同一 action；SystemManage CRUD 与 Authorizer 查到同一份数据。
5. 启动初始化在监听前完成；初始化失败时不进入可服务状态。
6. 新库自动创建全部表并写入两个内置角目录，重复启动不重复写入。
7. 多副本并发初始化不产生重复内置角或将唯一冲突误报为启动失败。
8. Manage RoleCode 签发、精确权限鉴权、菜单绑定与 09 真实 HTTP 测试在共享存储下通过。
