...
| `2b45346` | 菜单权限事务内原子替换 | 需要补入 | 当前 `updateMenuModelAll` 仍按权限数量判断、Remove/Add/Save 分步执行并吞掉错误；没有故障回滚、字段保留和真实落库测试 | `go test ./pkg/server/api/manage -count=1`; `git ls-tree main pkg/server/api/manage/menumanage_replace_fault_test.go` |
...
| `f86e5fe` | 示例 04 命令选中行语义 | 需要补入 | 当前 `ViewCommandModel` 对所有命令无条件设置 `IsSelectRow=true`，通用 Add/Edit/Remove 等命令被错误约束 | `go test ./examples/04-shop-performance/api/manage -count=1` |
...
| `9b4d475` | 保留 `decimal.Decimal` 搜索值 | 需要补入 | 当前 `convertOp1` 对 struct 类型返回零值且无错误，缺少 `decimal.NewFromString` 分支和回归测试 | `go test ./service/manage/view -count=1`; `git ls-tree main service/manage/view/model_test.go` |
