...
| 13. 持久化正确性与外部测试分离 | 进行中 |  | 已创建中文聚焦计划并确认 RED 基线：默认测试隐式连接 MySQL，且 SyncBatchDelay、pending、fatal-break 与零成功日志存在缺口 |
...
**状态：** 进行中。已创建中文聚焦计划 `docs/codex/plans/13-persistence-correctness.md`。当前 `go test ./pkg/persistence/... -count=1 -timeout=5m` 为预期 RED：多个默认测试连接 `127.0.0.1:3306`，并已复现默认批延迟、同步计数、pending、fatal-break 和虚假成功日志问题。首先执行任务 13.1 默认/外部测试分层。
