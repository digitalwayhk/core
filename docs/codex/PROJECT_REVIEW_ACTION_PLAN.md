...
| 13. 持久化正确性与外部测试分离 | 进行中 |  | 13.1-13.2 及 13.3a/b/d 已完成：默认/外部套件已分层，GORM result 错误已传播，SharedBadger 仅按 CAS 确认 key 计数与减 pending，零/部分/全部日志已分流；下一步完成 fatal-break 与纯 fake 回退测试 |
