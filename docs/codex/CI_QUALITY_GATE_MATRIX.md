...
| `observational/casdoor-rbac` | 观察（本地已通过） | 否 | PR、push | 10 分钟 | `./scripts/test.sh integration-casdoor-rbac` | 无 | examples/auth | 覆盖示例 05 的 Casdoor 双域登录、Webhook 与 Manage RBAC；本机热缓存约 29 秒、冷构建含依赖下载约 123 秒，连续稳定后评估提升 required |
