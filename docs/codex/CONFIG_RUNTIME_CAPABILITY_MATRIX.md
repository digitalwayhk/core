...
| Kafka、RabbitMQ 内建 provider | Kafka 同步写等待全部 ISR ACK，writer 允许在 Broker 开启 auto-create 时按动态 subject 建 topic；RabbitMQ persistent publish 等待 publisher confirm；两者可靠消费只在 Handler 成功后确认 | `KafkaProvider`、`RabbitMQProvider`；外部 Broker 与显式集成门禁。Connect 只验证 Broker 可达；若关闭 auto.create.topics，运维必须按 Prefix+规范化 subject 预建 topic | supported（Conditional） |
| Kafka、RabbitMQ ordered-reliable / keyed-reliable | 两者均未声明 `OrderedReliableMQProvider` 或 `KeyedReliableMQProvider`；requirement 或显式 `KeyConcurrency>1` 必须 fail closed，不得静默串行 | 无严格同键有序 / owner 内分键调度 | rejected |
...
| `ServerConfig.MQ.Kafka.StartOffset` | supported | Kafka provider | 默认 latest，仅新消费组无提交位移时生效；earliest 才从 topic 开头回放 |
