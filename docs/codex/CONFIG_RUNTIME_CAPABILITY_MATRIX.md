...
| Kafka、RabbitMQ 内建 provider | Kafka 同步写等待全部 ISR ACK，首次 Publish/Subscribe 声明 topic；RabbitMQ persistent publish 等待 publisher confirm，Subscribe 返回前完成 queue bind；两者可靠消费只在 Handler 成功后确认 | `KafkaProvider`、`RabbitMQProvider`；外部 Broker 与显式集成门禁。Connect 只验证 Broker 可达，动态 subject 由 Provider 声明 | supported（Conditional） |
