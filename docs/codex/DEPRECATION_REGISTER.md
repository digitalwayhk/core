...
| `smodels.UserPeermissionsModel`、`NewUserPeermissionsModel` | 消费方用户与 `ManageRoleModel.Code` 的关系模型 + Core `ManageRolePermissionModel` | v1.3.0 | v2.0.0 | server/smodels | 历史按用户绑定菜单权限 ID 的消费方 | `pkg/server/smodels/managerolemodel_test.go`、`examples/09-admin-manage-rbac` |
