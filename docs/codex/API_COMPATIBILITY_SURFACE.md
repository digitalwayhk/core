...
| `mq.NewKafkaProvider`、`mq.NewRabbitMQProvider`、两者的 `ReliableMQProvider` 实现 | Stable（加性，Conditional） | server/mq | 内建 Kafka/RabbitMQ 发布订阅与 Handler 成功后确认；不声明 ordered-reliable | provider/factory/race 测试；`scripts/test-external-mq.sh` 真实 Broker 门禁 |
