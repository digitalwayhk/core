...
两种内建 Provider 都是 at-least-once，不是 exactly-once；可靠 Handler 成功后才确认，业务必须继续用 EventID/Inbox 幂等。两者都不满足 `RequireOrderedReliableByShardKey`。默认 PR quick 不启动 Broker；真实门禁是手工/定时外部验证，未运行时必须报告 `NOT RUN`。
